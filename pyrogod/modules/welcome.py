...
from pyrogod.helpers.pyrohelper import get_arg, welcome_chat
...
async def welcome(client, message):
    await pyrogoddb.clear_welcome(str(message.chat.id))
...
async def new_welcome(client, message):
    msg_id = await pyrogoddb.get_welcome(str(message.chat.id))
    caption = ""
    men = ""
...
async def setwelcome(client, message):
    ...
    await pyrogoddb.save_welcome(str(message.chat.id), msg_id)
