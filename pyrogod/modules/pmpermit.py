...
from pyrogod.helpers.pyrohelper import get_arg, denied_users
import pyrogod.database.pmpermitdb as pyrogoddb
...
async def pmguard(client, message):
    ...
    if not arg:
        ...
        return
    if arg == "off":
        await pyrogoddb.set_pm(False)
        await message.edit("**PM Guard Deactivated**")
    if arg == "on":
        await pyrogoddb.set_pm(True)
...
async def pmguard(client, message):
    ...
    if not arg:
        ...
        return
    await pyrogoddb.set_limit(int(arg))
...
async def setpmmsg(client, message):
    ...
    if not arg:
        ...
        return
    if arg == "default":
        await pyrogoddb.set_permit_message(pyrogoddb.PMPERMIT_MESSAGE)
        await message.edit("**Anti_PM message set to default**.")
        return
    await pyrogoddb.set_permit_message(f"`{arg}`")
...
async def setpmmsg(client, message):
    ...
    if not arg:
        ...
        return
    if arg == "default":
        await pyrogoddb.set_block_message(pyrogoddb.BLOCKED)
        await message.edit("**Block message set to default**.")
        return
    await pyrogoddb.set_block_message(f"`{arg}`")
...
async def allow(client, message):
    ...
    pmpermit, pm_message, limit, block_message = await pyrogoddb.get_pm_settings()
    await pyrogoddb.allow_user(chat_id)
...
async def deny(client, message):
    chat_id = message.chat.id
    await pyrogoddb.deny_user(chat_id)
